# -*- coding: utf-8 -*-
"""
Strategia DFS (Depth-First Search) - przeszukiwanie wgłąb.

Strategia ŚLEPA (nieinformowana): nie używa heurystyki, rozwija wierzchołki
w kolejności odległości (liczby krawędzi) od startu. Frontier to stos LIFO.
...
NAZWA = "DFS (wgłąb)"
OPIS = "Strategia ślepa: przeszukiwanie wszerz, frontier = stos LIFO."
ETYKIETA_FRONTIERA = "Stos LIFO"


def przeszukaj(G, start, goal, tryb_cykli="multiple", limit_krokow=1000):
    """
    Generator realizujący przeszukiwanie DFS.
    ...
    Frontier (stos LIFO) przechowuje całe ŚCIEŻKI, więc w każdym kroku
    ...
    while frontier:
        ...
        # Pobieramy najstarszą ścieżkę ze stosu (to czyni z tego DFS).
        sciezka = frontier.pop()
        wezel = sciezka[-1]
        ...
        # --- Test celu (wykonywany przy zdejmowaniu ze stosu) ---
        if wezel == goal:
            yield {
                "krok": krok,
                "status": "found",
    ...
    # Stos pusty, a celu nie znaleziono.
    yield {
        "krok": krok,
        "status": "brak",
        "current": None,
